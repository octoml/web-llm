        ...
        for gv in mod.functions:
            ...
            if isinstance(func, relax.Function):
                mod[gv] = func.with_attr(
                    "tir_var_upper_bound",
                    {"n": config.max_sequence_length, "m": config.max_sequence_length},
                )
...
def get_params(config, model):
    ...
    for _, param in model.named_parameters():
        param_list.append(
            tvm.nd.array(param.detach().cpu().numpy().astype(config.dtype), tvm.cpu())
        )
    ...
    param_list.append(tvm.nd.array(np.cos(emb).astype(config.dtype), tvm.cpu()))
    param_list.append(tvm.nd.array(np.sin(emb).astype(config.dtype), tvm.cpu()))
    ############ End ############
...
def mod_transform_before_build(
    ...
    debug_dump_script(mod, "mod_transform_beofre_build.py", args)
    if args.dtype == "int4":
...
def build(mod_deploy: tvm.IRModule, args: Dict) -> None:
    ...
    if target_kind != "cpu":
        ...
        with db, tvm.target.Target("nvidia/nvidia-a10g"):
            # TODO(xiyou): enable this
    ...
    if target_kind == "webgpu":
        ...
        output_filename = f"{args.model}_{target_kind}_{args.dtype}.so"
